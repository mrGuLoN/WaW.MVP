...
namespace LevelGenerator.Scripts
{
    ...
    {
        [SerializeField] private Transform _generatedStage, _bakeStage;
        /// <summary>
        /// LevelGenerator seed
        /// </summary>
        public int Seed;
        ...
        public void RetransformLight()
        {
            var lights = _generatedStage.GetComponentsInChildren<Light>();
            foreach (var l in lights)
            {
                l.transform.SetParent(_bakeStage);
            }

            var probes = _generatedStage.GetComponentsInChildren<LightProbeGroup>();
            foreach (var l in probes)
            {
                l.transform.SetParent(_bakeStage);
            }
        }

        public void RetransformWall2D()
        {
            var walls = _generatedStage.GetComponentsInChildren<Collider2D>();
            foreach (var wall in walls)
            {
                wall.transform.SetParent(_bakeStage);
            }
        }
